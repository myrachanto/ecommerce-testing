...
func (r *categoryrepository) Create(category *model.Category) (*httperrors.HttpError) {
	...
	category.Code = code
	//get majorcategory code
	majorcatcode := category.Majorcat
	who := bson.M{"code": majorcatcode}
	fmt.Println(who)
	// collection1 := db.Collection("majorcategory")
	collection1 := db.Collection("majorcategory")
	// PushToArray := bson.M{"$push": bson.M{"loginattempts": bson.M{"timestamp": 1464045212, "ip": "195.0.0.200"}}}
	d, errs := collection1.UpdateOne(ctx, who, bson.M{"$push": bson.M{"category": category }})
 fmt.Println(d, "tryiiiiiiiiiiiiiiiiiiiiiiiiiii")
	// collection.Update(Who, PushToArray)
	// _, errs := collection1.UpdateOne(ctx, who, data)
		if errs != nil {
			return httperrors.NewBadRequestError(fmt.Sprintf("Create category Failed, %d", errs))
	}
	DbClose(c)
	return nil
}
